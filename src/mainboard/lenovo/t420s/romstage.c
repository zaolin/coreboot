...
static void rcba_config(void) // Root Complex Base Address Register
{
	 ...
	 */
	 /*
	 0x3100: 0x03243200 (INTB << D31IP_TTIP) | (INTB << D31IP_SIP2) | (INTB << D31IP_SMIP) | (INTB << D31IP_SIP);
	0x3108: 0x00014321
	0x310c: 0x43214321 (INTD << D28IP_P8IP) | (INTC << D28IP_P7IP) | (INTB << D28IP_P6IP) | (INTA << D28IP_P5IP) | (INTD << D28IP_P4IP) | (INTC << D28IP_P3IP) | (INTB << D28IP_P2IP) | (INTA << D28IP_P1IP);
	0x3110: 0x00000001
	0x3114: 0x40004321
	0x3118: 0x00000001
	0x311c: 0x00000001
	...
	RCBA32(D31IP) = (INTB << D31IP_TTIP) | (INTB << D31IP_SIP2) |
			(INTB << D31IP_SMIP) | (INTB << D31IP_SIP);
	...
	RCBA32(D27IP) = (INTA << D27IP_ZIP); // hda, default
	RCBA32(D26IP) = (INTA << D26IP_E2P); // ehci, default
	RCBA32(D25IP) = (NOINT << D25IP_LIP); // gbe lan pin
...
*/
	...
	DIR_ROUTE(D31IR, PIRQA, PIRQC, PIRQD, PIRQA); // 0230
	DIR_ROUTE(D29IR, PIRQC, PIRQA, PIRQD, PIRQH); // 2037
	DIR_ROUTE(D28IR, PIRQD, PIRQC, PIRQB, PIRQA); // 3210
	DIR_ROUTE(D27IR, PIRQD, PIRQC, PIRQB, PIRQG); // 3216
	DIR_ROUTE(D26IR, PIRQD, PIRQC, PIRQF, PIRQA); // 3250
	DIR_ROUTE(D25IR, PIRQH, PIRQG, PIRQF, PIRQE); // 7654
	DIR_ROUTE(D22IR, PIRQB, PIRQC, PIRQD, PIRQA); // 1230
...
void main(unsigned long bist)
{
	...
	struct pei_data pei_data = {
		...
		.usb_port_config = {
			...
			{ 0, 0, 0x0000 }, /* P0 RESERVED */
			{ 1, 1, 0x0080 }, /* P1 SYSTEM PORT 2 (To system port) OC 1*/
			{ 1, 0, 0x0080 }, /* P2: HALF MINICARD (WLAN) no oc*/
			{ 1, 0, 0x0080 }, /* P3: WWAN, no OC */
			{ 1, 0, 0x0040 }, /* P4: SMART CARD SLOT */
			{ 1, 0, 0x0080 }, /* P5: Expresscard, no OC */
			{ 0, 0, 0x0000 }, /* P6: RESERVED */
			{ 0, 0, 0x0000 }, /* P7: RESERVED */
			{ 0, 0, 0x0000 }, /* P8: TOUCH PANEL ? */
			{ 1, 5, 0x0080 }, /* P9: SYSTEM PORT 1 (To USBAO), OC 5 */
			{ 1, 0, 0x0040 }, /* P10: fingerprint reader, no OC */
			{ 1, 0, 0x0040 }, /* P11: BLUETOOTH, no OC. */
			{ 1, 0, 0x0080 }, /* P12: DOCKING, no OC */
			{ 1, 0, 0x0080 }, /* P13: USB CAMERA (LCD), no OC */
		},
